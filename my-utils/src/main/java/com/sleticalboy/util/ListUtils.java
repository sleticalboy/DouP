...
public final class ListUtils {
    ...
    public static <V> void relocationItem(List<V> sourceList, int from, int to) {
        if (isEmpty(sourceList)) {
            return ;
        }

        V item = sourceList.remove(from);
        sourceList.add(to, item);
    }

    public static <V> void exchangeItem(List<V> sourceList, int fromPos, int desPos) {
        if (isEmpty(sourceList)) {
            return;
        }
        V fromItem = sourceList.get(fromPos);
        V desItem = sourceList.get(desPos);
        sourceList.set(fromPos, desItem);
        sourceList.set(desPos, fromItem);
    }
}
